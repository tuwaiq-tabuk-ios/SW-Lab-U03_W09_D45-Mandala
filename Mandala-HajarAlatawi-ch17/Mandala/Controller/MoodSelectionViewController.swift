...
//  Mandala
//
//  Created by HAJAR on 09/12/2021.
//
...
class MoodSelectionViewController: UIViewController {
  ...
  var moods: [Mood] = [] {
    ...
    didSet {
      ...
      moodButtons = moods.map { mood in
        ...
        moodButton.setImage(mood.image,
                            for: .normal)
  ...
  override func viewDidLoad() {
    ...
    addMoodButton.layer.cornerRadius
    = addMoodButton.bounds.height / 2
  ...
  var currentMood: Mood? {
    ...
    didSet {
      guard let currentMood = currentMood else {
        addMoodButton?.setTitle(nil,
                                for: .normal)
        addMoodButton?.backgroundColor = nil
        return
      }
      
      addMoodButton?.setTitle("I'm \(currentMood.name)",
                              for: .normal)
  ...
  @IBAction func addMoodTapped(_ sender: Any) {
      ...
      let newMoodEntry = MoodEntry(mood: currentMood,
                                   timestamp: Date())
